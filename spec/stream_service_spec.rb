...
describe StreamService do
  let(:service) { StreamService::Service.new("http://192.168.99.100:8080") }
  ...
  it 'can get stuff!' do
    ...
    expect(response[:stream_items].first.stream_id).to eq "abc123"
  ...
  it 'can grab content from multiple users' do
    ...
    expect(response1[:stream_items].first["stream_id"]).to eq "abc123"
    ...
    expect(response2[:stream_items].first["stream_id"]).to eq "malory"
  ...
  it 'can grab content from multiple users with a limit and pagination slug' do
    ...
    expect(response[:stream_items].first["stream_id"]).to eq "abc123"
    expect(response[:stream_items].first["type"]).to eq 0
    expect(response[:stream_items].any? { |item| item["stream_id"] == "asdf" }).to be false
    expect(response[:stream_items].any? { |item| item["stream_id"] == "pizza" }).to be false
    ...
    expect(next_response[:stream_items].first["stream_id"]).to eq "asdf"
    expect(next_response[:stream_items].first["ts"].mday).to eq t.mday
    expect(next_response[:stream_items].first["type"]).to eq 0
    expect(next_response[:stream_items].any? { |item| item["stream_id"] == "abc123" }).to be false
    expect(next_response[:stream_items].any? { |item| item["stream_id"] == "pizza" }).to be false
  end
end
